...
#define EMBED(N) \
  CSynapseClient* SYNAPSE_DLL_EXPORT N##_EnumerateInterfaces(const char* version, CSynapseServer * pServer); \
  { \
    Str nm(#N); \
    EmbedInterfaces(nm, N##_EnumerateInterfaces); \
  }
...
bool CSynapseServer::Initialize( const char* conf_file, PFN_SYN_PRINTF_VA pf ){
	...
	*/
	...
	Str xmlSrc;
	xmlSrc += "<?xml version='1.0' encoding='iso-8859-1' standalone='yes'?>";
	xmlSrc += "<!-- synapse XML configuration -->";
	xmlSrc += "<synapseconfig>";
	xmlSrc += "  <client name='core'>";
	xmlSrc += "    <!-- NOTE: not all idtech3 engine support png format! ioq3 does though, so at this point we assume it's fair to enable. See https://github.com/TTimo/GtkRadiant/issues/42 -->";
	xmlSrc += "    <api name='image'>";
	xmlSrc += "      png jpg tga bmp";
	xmlSrc += "    </api>";
	xmlSrc += "    <api name='VFS'>";
	xmlSrc += "      pk3";
	xmlSrc += "    </api>";
	xmlSrc += "    <api name='shaders'>";
	xmlSrc += "      quake3";
	xmlSrc += "    </api>";
	xmlSrc += "    <api name='map'>";
	xmlSrc += "      mapq3";
	xmlSrc += "    </api>";
	xmlSrc += "    <api name='eclass'>";
	xmlSrc += "      def";
	xmlSrc += "    </api> ";
	xmlSrc += "    <api name='surfdialog'>";
	xmlSrc += "      quake3";
	xmlSrc += "    </api>";
	xmlSrc += "  </client>";
	xmlSrc += "  <client name='image'>";
	xmlSrc += "    <api name='VFS'>";
	xmlSrc += "      pk3";
	xmlSrc += "    </api>";
	xmlSrc += "  </client>";
	xmlSrc += "  <client name='shaders'>";
	xmlSrc += "    <!-- NOTE: this is a SYN_PROVIDE -->";
	xmlSrc += "    <api name='shaders'>";
	xmlSrc += "       quake3";
	xmlSrc += "    </api>";
	xmlSrc += "    <api name='VFS'>";
	...
	xmlSrc += "    </api>";
	xmlSrc += "  </client>";
	xmlSrc += "  <client name='map'>";
	xmlSrc += "    <api name='shaders'>";
	xmlSrc += "      quake3";
	xmlSrc += "    </api>";
	xmlSrc += "  </client>";
	xmlSrc += "  <client name='xmap'>";
	xmlSrc += "    <api name='shaders'>";
	xmlSrc += "      quake3";
	xmlSrc += "    </api>";
	xmlSrc += "  </client>";
	xmlSrc += "  <client name='model'>";
	xmlSrc += "    <api name='shaders'>";
	xmlSrc += "      quake3";
	xmlSrc += "    </api>";
	xmlSrc += "    <api name='VFS'>";
	...
	mpDoc = xmlParseMemory(xmlSrc.GetBuffer(), xmlSrc.GetLength());
...
#elif defined( __linux__ ) || defined( __FreeBSD__ ) || defined( __APPLE__ ) || defined( USE_POSIX )
	...
	*/
	...
	/*
	CSynapseClient* SYNAPSE_DLL_EXPORT imagepng_EnumerateInterfaces(const char* version, CSynapseServer * pServer);
	Str nm("imagepng");
	EmbedInterfaces(nm, imagepng_EnumerateInterfaces);

	CSynapseClient* SYNAPSE_DLL_EXPORT map_EnumerateInterfaces(const char* version, CSynapseServer * pServer);
	nm = ("map");
	EmbedInterfaces(nm, map_EnumerateInterfaces);

	CSynapseClient* SYNAPSE_DLL_EXPORT eclassfgd_EnumerateInterfaces(const char* version, CSynapseServer * pServer);
	nm = ("eclassfgd");
	EmbedInterfaces(nm, eclassfgd_EnumerateInterfaces);

	CSynapseClient* SYNAPSE_DLL_EXPORT entity_EnumerateInterfaces(const char* version, CSynapseServer * pServer);
	nm = ("entity");
	EmbedInterfaces(nm, entity_EnumerateInterfaces);
	*/
	EMBED(entity);
	EMBED(image);
	EMBED(imagepng);
...
#ifdef USE_POSIX
void CSynapseServer::EmbedInterfaces(Str& soname, PFN_SYNAPSE_ENUMERATEINTERFACES ep) {
...
const char* CSynapseClient::GetInfo(){
	return "CSynapseClient built " __DATE__;
}
