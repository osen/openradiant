 ...
 * CGameDescription
 *
 * Create a default game description.
 * TODO: base game set to "root". This will need to be the directory containing .map.
 */
...
bool noMapsInHome;   ///< set this if you want to open the engine path/base dir/maps dir for map open/save dialoges
	...
	*/
	...
	idTech2 = false;
	mGameName = "Generic";
	mCaulkShader = SHADER_NOT_FOUND;

	std::string basePath = Env_ProjectRoot() + "/";

	mEnginePath = basePath.c_str();
	//mBaseGame = Env_GetProjectRoot().c_str();
	mBaseGame = "unused";
}
...
/*
	...
	mGlobalPrefs.GetPref( "autoload", &m_bAutoLoadGame, true );
...
void CGameDialog::DummyScanForGames()
{
	if (!mGames.empty()) {
	...
	mGames.push_front(new CGameDescription());
...
*/
	...
	m_inipath = g_string_new(iniPath.c_str());
}
...
*/
...

			{
				GtkTreeIter group;
				gtk_tree_store_append( store, &group, NULL );
				gtk_tree_store_set( store, &group, 0, _( "Other" ), 1, PTAB_BRUSH, -1 );
/*
				{
					GtkTreeIter tab;
					gtk_tree_store_append( store, &tab, &group );
...
void PrefsDlg::LoadPrefs(){
	...
	//mLocalPrefs.ReadXMLFile("D:\\local.prefs");
	...
	mLocalPrefs.GetPref( LOADLAST_KEY,           &m_bLoadLast,           FALSE );
	mLocalPrefs.GetPref( BSP_KEY,                &m_bInternalBSP,        FALSE );
	mLocalPrefs.GetPref( RCLICK_KEY,             &m_bRightClick,         TRUE );
	mLocalPrefs.GetPref( AUTOSAVE_KEY,           &m_bAutoSave,           FALSE );
	...
	mLocalPrefs.GetPref( SAVEBEEP_KEY,           &m_bSaveBeep,           FALSE );
	...
	mLocalPrefs.GetPref( WIDETOOLBAR_KEY,        &m_bLatchedWideToolbar,                FALSE );
	m_bWideToolbar = m_bLatchedWideToolbar;

	mLocalPrefs.GetPref( PLUGINTOOLBAR_KEY, &m_bLatchedPluginToolbar, FALSE );
	...
	mLocalPrefs.GetPref( FIXEDTEXSIZE_KEY,       &m_bFixedTextureSize,          TRUE );
	...
	mLocalPrefs.GetPref( SHOWTEXDIRLIST_KEY,     &m_bShowTexDirList,             FALSE );
	...
	mLocalPrefs.GetPref( WIDTH_KEY,              &mWindowInfo.position.w,        1024 );
	mLocalPrefs.GetPref( HEIGHT_KEY,             &mWindowInfo.position.h,        600 );
	...
	vec3_t vDefaultColours[COLOR_LAST] = {
		{0.25f, 0.25f,  0.25f},
		{0.054902f,   0.074510f,    0.164706f},
		{0.082353f, 0.109804,  0.243137f},
		{0.082353f,  0.109804f,   0.243137f},
		{0.25f, 0.25f,  0.25f},
		{0.0f,  0.0f,   0.0f},
		{0.f,   0.f,    1.f},
		{0.f,   0.f,    0.f},
