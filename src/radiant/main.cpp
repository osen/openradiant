...
static void create_splash() {
    ...
    GtkWidget* image = image_new_from_horde("bitmaps/splash.png");
...
#ifdef _WIN32
	size_t size = 0;
	const unsigned char *data = Env_ReadFile("themes/MS-Windows/gtk-2.0/gtkrc", &size);
	if (data)
	{
		gtk_rc_parse_string((gchar*)data);
	}
#endif
