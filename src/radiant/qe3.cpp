...
bool QE_LoadProject( const char *projectfile ){
	...
	xmlSrc += "<!DOCTYPE project SYSTEM 'dtds / project.dtd'>";
	xmlSrc += "<project>";
	xmlSrc += "<key name='version' value='2'/>";
	xmlSrc += "<key name='template_version' value='8'/>";
	xmlSrc += "<key name='basepath' value='$TEMPLATEenginepath$TEMPLATEbasedir/'/>";
	xmlSrc += "<key name='rshcmd' value=''/>";
	xmlSrc += "<key name='remotebasepath' value='$TEMPLATEenginepath$TEMPLATEbasedir/'/>";
	xmlSrc += "<key name='entitypath' value='$TEMPLATEtoolspath$TEMPLATEbasedir/scripts/entities.def'/>";
	xmlSrc += "<key name='texturepath' value='$TEMPLATEenginepath$TEMPLATEbasedir/textures/'/>";
	xmlSrc += "<key name='autosave' value='$TEMPLATEuserhomepath$TEMPLATEbasedir/maps/autosave.map'/>";
	xmlSrc += "<key name='mapspath' value='$TEMPLATEuserhomepath$TEMPLATEbasedir/maps/'/>";
	xmlSrc += "</project>";

	//doc = ParseXMLFile( projectfile, true );
	doc = xmlParseMemory(xmlSrc.GetBuffer(), xmlSrc.GetLength());
...
extern "C" void Sys_FPrintf_VA( int level, const char *text, va_list args ) {
	...
	if ( level != SYS_NOCON ) {
		...
		if ( g_qeglobals_gui.d_edit != NULL ) {
			...
			GtkTextMark* end = gtk_text_buffer_create_mark( buffer, "end", &iter, FALSE );
