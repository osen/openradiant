...
void MainFrameSetExportEnabled(bool enabled)
{
	if(exportItem)
	{
		gtk_widget_set_sensitive(exportItem, enabled);
	}

	if(bakeItem)
	{
		gtk_widget_set_sensitive(bakeItem, enabled);
	}
}
...
gint HandleCommand( GtkWidget *widget, gpointer data ){
	...
	if ( id >= CMD_TEXTUREWAD && id <= CMD_TEXTUREWAD_END ) {
		...
		g_pParentWnd->ActiveXY()->OnEntityCreate( str );
		  ...
		  case ID_COLOR_SETOPEN: g_pParentWnd->OnColorSetopen(); break;
		  ...
		  case ID_FILE_EXPORT: export_obj(false, true); break;
		  case ID_FILE_BAKE: export_obj(true, true); break;
		  }}
...
/*
	...
	menu_separator( menu );
*/
	if(Env_Extension("export_obj"))
	{
		item = create_menu_item_with_mnemonic(menu, _("_Export .obj"),
			G_CALLBACK(HandleCommand), ID_FILE_EXPORT);
		exportItem = item;
		item = create_menu_item_with_mnemonic(menu, _("_Bake .obj"),
			G_CALLBACK(HandleCommand), ID_FILE_BAKE);
		bakeItem = item;
...
*/
	item = create_menu_item_with_mnemonic( menu, _( "Flush & Reload" ),
...
*/
	menu_separator( menu );
	item = create_menu_item_with_mnemonic(menu, _("View _Textures"), G_CALLBACK(HandleCommand), ID_VIEW_TEXTURE);
...
*/
	...
	create_menu_item_with_mnemonic( menu_3, _("OpenRadiant Original"), G_CALLBACK(HandleCommand), ID_COLOR_SETOPEN);
...
*/
	...
	create_menu_item_with_mnemonic( menu, _( "_About OpenRadiant" ),
...
*/
	// TODO KARSTEN
	window_set_icon_from_horde(window, "bitmaps/icon.png");
#endif
...
void MainFrame::OnColorSetopen(){
	...
	g_qeglobals.d_savedinfo.colors[COLOR_GRIDBACK][0] = 0.054902f;
	g_qeglobals.d_savedinfo.colors[COLOR_GRIDBACK][1] = 0.074510f;
	g_qeglobals.d_savedinfo.colors[COLOR_GRIDBACK][2] = 0.164706f;

	g_qeglobals.d_savedinfo.colors[COLOR_GRIDMAJOR][0] = 0.082353f;
	g_qeglobals.d_savedinfo.colors[COLOR_GRIDMAJOR][1] = 0.109804f;
	g_qeglobals.d_savedinfo.colors[COLOR_GRIDMAJOR][2] = 0.243137f;

	g_qeglobals.d_savedinfo.colors[COLOR_GRIDMINOR][0] = 0.082353f;
	g_qeglobals.d_savedinfo.colors[COLOR_GRIDMINOR][1] = 0.109804f;
	g_qeglobals.d_savedinfo.colors[COLOR_GRIDMINOR][2] = 0.243137f;
