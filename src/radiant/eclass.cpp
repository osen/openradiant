...
static eclass_t *addEntity(const char *name, float r, float g, float b, bool brush)
{
	eclass_t* e = Eclass_ForName(name, brush);
	e->color[0] = r;
	e->color[1] = g;
	e->color[2] = b;
...
void Eclass_Init(){
	...
	// TODO KARSTEN add default entities
	//eclass = EClass_Create("light", 0, 0, 0, NULL, NULL, NULL);
	//strcpy(eclass->flagnames[0], "tickbox1");
	//strcpy(eclass->flagnames[1], "tickbox2");
	//strcpy(eclass->flagnames[2], "tickbox3");

	//EClass_Create("entity", 0, 0, 0, NULL, NULL, NULL);

	eclass = addEntity("point", 0, 0.5f, 0, false);
	addEntity("area", 0, 0, 0.75f, true);
	addEntity("light", 0.75f, 0.75f, 0, false);
