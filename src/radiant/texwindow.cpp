...
void Texture_ListDirectory(){
	...
	sprintf( dirstring, "%s", texture_directory );

	if (strcmp(dirstring, "./") == 0 || strcmp(dirstring, "/") == 0)
	{
		strcpy(dirstring, "");
	}
	...
	for ( temp = files; temp; temp = temp->next )
	{
		sprintf( name, "%s%s", dirstring, (char*)temp->data );
 ...
 * FillTextureList
 *
 * Populate the initial texture directory list.
 * TODO: For now it is just hardcoded to "./" so that the current project directory
 * is scanned.
 */
void FillTextureList( GSList** pArray )
{
	...
	if ( !g_PrefsDlg.m_bTexturesShaderlistOnly) {
		std::string prefix = Env_GetPrefix();
		//texdirs = g_slist_append(texdirs, g_strdup(prefix.c_str()));
		texdirs = g_slist_append(texdirs, g_strdup(""));
		texdirs_tmp = vfsGetDirList( "" );
		...
		for ( p = texdirs_tmp; p; p = g_slist_next( p ) )
		{
			...
			{
				Str str;
				str.Format("%s", (char*)p->data);
				texdirs = g_slist_append( texdirs, g_strdup(str.GetBuffer()) );
			}
			else
			{
...
void Texture_Init(){
	...
	g_qeglobals.d_texturewin.texdef.SetName(SHADER_CAULK);
	g_qeglobals.d_texturewin.pShader = QERApp_Shader_ForName(SHADER_CAULK);
}
